...
const MapView = ({ clinics = [], userLocation = null }) => {
  const [selected, setSelected] = useState(null);
  const { isLoaded } = useJsApiLoader({ googleMapsApiKey });

  // Center on first clinic or user location
  const center = useMemo(() => {
    if (clinics.length > 0 && clinics[0].lat && clinics[0].lng) {
      return { lat: clinics[0].lat, lng: clinics[0].lng };
    }
